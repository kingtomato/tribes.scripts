...
function King::Settings::ExportSave()
{
	...
	%l = 0;
	$King::Settings::Log[%l++] = "----------------------------------------------------------------------------------------------------";
	$King::Settings::Log[%l++] = "------------------------------ KINGTOMATO SYSTEM SETTINGS (EXPORTED) -------------------------------";
	$King::Settings::Log[%l++] = "----------------------------------------------------------------------------------------------------";
	export("$King::Settings::Log*", %logfile, false);

	// Kill the "no delete a friend"
	deleteVariables("$KingPref::Friends*");

	for (%a = 1; $King::Settings::Export[%a] != ""; %a++)
		export($King::Settings::Export[%a], %logfile, true);

	%l = 0;
	$King::Settings::Log[%l++] = "----------------------------------------------------------------------------------------------------";
	$King::Settings::Log[%l++] = "------------------------------ KINGTOMATO SYSTEM SETTINGS (EXPORTED) -------------------------------";
	$King::Settings::Log[%l++] = "----------------------------------------------------------------------------------------------------";
	export("$King::Settings::Log*", %logfile, true);
}
