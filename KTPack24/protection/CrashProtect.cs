...
if (KingPref::Enabled(CrashProtect))
{
	...
	Event::Attach(eventTopPrint, PrintProtect::Catch);
	Event::Attach(eventCenterPrint, PrintProtect::Catch);
	Event::Attach(eventBottomPrint, PrintProtect::Catch);
}
...
function PrintProtect::Catch(%msg, %delay)
{
	...
	{
		%bitmap = String::getSubStr(%msg, %tag_open + 3, %tag_close - %tag_open + 7);
		echo("Crash Attempt: Print using bitmap: " @ %bitmap);
		return mute;
	}
}
