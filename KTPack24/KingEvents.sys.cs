...
function remoteCP(%manager, %msg, %timeout) {
	if (%manager == 2048) {
		...
		Client::centerPrint(PrintProtect::Clean(%msg), 0);
...
function remoteBP(%manager, %msg, %timeout) {
	if (%manager == 2048) {
		...
		if (%timeout)
			schedule("clearCenterPrint(" @ $centerPrintId @ ");", %timeout);
		Client::centerPrint(PrintProtect::Clean(%msg), 1);
...
function remoteTP(%manager, %msg, %timeout) {
	if (%manager == 2048) {
		...
		if (%timeout)
			schedule("clearCenterPrint(" @ $centerPrintId @ ");", %timeout);
		Client::centerPrint(PrintProtect::Clean(%msg), 2);
...
function remoteITXT(%manager, %msg) {
	...
	Event::Trigger(eventInventoryText, %msg);
}
