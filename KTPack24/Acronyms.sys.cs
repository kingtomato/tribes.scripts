...
// instead of exporting them to the settings file, I am now exporting them to the Acronyms.cs
// file in the config\\King_Acronyms.cs file.

Event::Attach(eventExit, Acronym::Save);
function Acronym::Save()
{
	// create a new object to save the file to
	%obj = newObject("AcronymList", FearGuiFormattedText, 0,0,0,0);
	...
	// Create the file
	%l = 0;
	%f[%l++] = "// ^^^ Ignore Above Lines ^^^";
	%f[%l++] = "// ------------------------------------------------------------------------------------------------";
	%f[%l++] = "// KingTomato Acronyms";
	%f[%l++] = "// --";
	%f[%l++] = "// Modify the below list to change your acronyms";
	%f[%l++] = "// ------------------------------------------------------------------------------------------------";

	for (%a = 1; %a <= $Acronym::Count; %a++)
	{
		%acro = $Acronym::List::[%a];
		%meaning = $Acronym::List[%acro];

		%f[%l++] = "Acronym::Add(\"" @ %acro @ "\",\t\"" @ escapestring(escapestring(%meaning)) @ "\");";
	}
	
	%f[%l++] = "// EOF";

	// Store it all in file
	for (%l = 1; (%line = %f[%l]) != ""; %l++)
		addExportText(%line);

	// export it
	exportObjectToScript("AcronymList", "config\\King_Acronyms.cs", true);

	// Remove object
	deleteObject(nameToID("AcronymList"));
	flushExportText();
...
// Do not edit the acronyms below. Edit them instead in your config directory, king_Acronyms.cs
// If the file doesn't exist yet, run tribes once, exit, then look for it.
//Acronym::Add("",	"");

if (!isFile("config\\King_Acronyms.cs"))
{
...
Acronym::Add("roflmao",	"Rolling On Floor Laughing My Ass Off");
Acronym::Add("rtfm",	"Read the Fucking Manual");
Acronym::Add("rtm",	"Read the Manual");
Acronym::Add("scorp",	"Scorpius");
...
Acronym::Add("tmi",	"Too Much Information");
...
Acronym::Add("wtmi",	"Way Too Much Information");
Acronym::Add("yw",	"Your Welcome");
}
else
	exec("King_Acronyms.cs");